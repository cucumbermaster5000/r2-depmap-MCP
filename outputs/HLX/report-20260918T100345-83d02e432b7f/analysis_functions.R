...
function (cfg, gene) 
{
    ...
    data$is_medulloblastoma <- tolower(data$OncotreePrimaryDisease) == 
        "medulloblastoma"
    ...
    result <- list(data = data, provenance = c(files$provenance, 
        list(gene_column = effect_col, model_selection = "Exact OncotreePrimaryDisease=Medulloblastoma; no subgroup inferred from model names", 
...
function (data, cutoff, cohort) 
{
    ...
    p <- ggplot2::ggplot(curves, ggplot2::aes(time, survival, 
        ...
        ggplot2::geom_step(ggplot2::aes(y = lower), linetype = 3, 
            alpha = 0.40000000000000002) + ggplot2::geom_step(ggplot2::aes(y = upper), 
        linetype = 3, alpha = 0.40000000000000002) + ggplot2::geom_point(data = curves[curves$censored > 
        0, ], shape = 3, size = 1.8) + ggplot2::scale_color_manual(values = c(High = "#b43b47", 
